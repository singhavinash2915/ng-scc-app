...
function FormBlocks({ form }: { form: FormResult[] | FormResult[] | undefined }) {
  ...
  return (
    <span className="inline-flex gap-1">
      {ordered.map((r, i) => (
        <span key={i} className={`w-7 h-7 r-card flex items-center justify-center t-meta font-black text-slate-900 dark:text-white ${
...
function AchievementCard({ a }: { a: Achievement }) {
  ...
  return (
    <div className={`relative overflow-hidden r-card p-4 transition-all ${
                     a.unlocked ? '' : 'glass opacity-60'}`}
         style={a.unlocked ? { background: tierColor } : undefined}>
      {a.unlocked && (
      ...
      <div className="relative">
        <div className="flex items-start justify-between">
          ...
          {a.tier && a.unlocked && (
            <span className="t-micro uppercase font-black tracking-wider px-1.5 py-0.5 rounded-full bg-white/10 text-slate-500 dark:text-white/80">
              {a.tier}
            </span>
          )}
        </div>
        ...
        {a.unlocked && (
          <div className="absolute top-2 right-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400" fill="currentColor" />
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div className="p-4 lg:p-8 space-y-5 max-w-5xl mx-auto">
        ...
        {/* ── HERO ─────────────────────────────────────────────────────── */}
        {/* A player's own page, on the house surface. The club green stays as
            a wash behind the top corner and a hairline, rather than a navy slab
            the rest of the app stopped using. */}
          ...
          <div className="relative p-6 lg:p-8 flex flex-col sm:flex-row items-center sm:items-start gap-5">
            {member.avatar_url ? (
              <img src={member.avatar_url} alt={member.name}
                   className="w-24 h-24 lg:w-28 lg:h-28 r-card object-cover ring-1 ring-emerald-400/40 flex-shrink-0" />
            ) : (
            ...
            <div className="flex-1 min-w-0 text-center sm:text-left">
              ...
              <h1 className="font-display font-extrabold text-slate-900 dark:text-white
                             text-2xl lg:text-3xl tracking-tight mt-1.5">
                {member.name}
              </h1>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
                <Pill v={memberStats?.batting_runs || 0} label="Runs" color="text-sky-600 dark:text-sky-300" />
                <Pill v={memberStats?.bowling_wickets || 0} label="Wickets" color="text-rose-600 dark:text-rose-300" />
                <Pill v={dismissals} label="Dismissals" color="text-emerald-600 dark:text-emerald-300" />
                <Pill v={`${winRate}%`} label="Win Rate" color="text-amber-600 dark:text-amber-300" />
              </div>
        ...
        {tab === 'overview' && (
          <div className="space-y-4">
            ...
            {!memberStats && view === 'season' ? (
              ...
              <div className="glass r-card relative overflow-hidden p-5">
                ...
                <div className="flex items-center justify-between mb-3 relative">
                  <h3 className="t-micro font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-[3px]">
                    {view === 'season' ? seasonLabel(CURRENT_SEASON) : 'All seasons'}
                  </h3>
                  <div className="flex r-control overflow-hidden border border-slate-200 dark:border-white/15">
                    {([['season', 'Season'], ['career', 'All time']] as const).map(([k, label]) => (
                      <button key={k} onClick={() => setView(k)}
                        className={`px-2.5 py-1 t-micro font-black uppercase tracking-wider ${
                          view === k ? 'bg-emerald-500 text-white'
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                {topKnocks.length > 0 && (
                  ...
                  <div className="absolute inset-x-0 top-0 h-[3px] bg-sky-400/70" />
                    <h3 className="t-micro font-black text-sky-600 dark:text-sky-300 uppercase tracking-[3px] mb-3 flex items-center gap-1.5 relative">
                      <TrendingUp className="w-3 h-3" /> Top 3 Knocks
                    </h3>
                    <div className="space-y-2 relative">
                      {topKnocks.slice(0, 3).map((k, i) => (
                        <div key={i} className="flex items-baseline gap-3 text-slate-900 dark:text-white">
                ...
                {topSpells.length > 0 && (
                  ...
                  <div className="absolute inset-x-0 top-0 h-[3px] bg-rose-400/70" />
                    <h3 className="t-micro font-black text-rose-600 dark:text-rose-300 uppercase tracking-[3px] mb-3 flex items-center gap-1.5 relative">
            ...
            <div className="glass r-card relative overflow-hidden p-5">
                  <div className="absolute inset-x-0 top-0 h-[3px] bg-emerald-400/70" />
              <h3 className="t-micro font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-[3px] mb-2 relative">
                Skill Radar
              </h3>
              <div className="relative">
                  ...
                  <a key={p.id} href={p.photo_url} target="_blank" rel="noopener noreferrer"
                    ...
                    {p.match && (
                      <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent p-2">
                        <p className="t-micro text-slate-900 dark:text-white font-bold truncate">vs {p.match.opponent || '—'}</p>
                        <p className="t-micro text-slate-500 dark:text-white/60">{new Date(p.match.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: '2-digit' })}</p>
                      </div>
                    )}
                  </a>
                ))}
              ...
              <div className="space-y-1.5">
                {matchesPlayed.slice(0, 50).map(m => (
                  <Link key={m.id} to="/matches" className="flex items-center gap-3 px-3 py-2.5 r-card hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    <div className={`w-7 h-7 r-card flex items-center justify-center text-xs font-black text-slate-900 dark:text-white flex-shrink-0 ${
    ...
    <div className="glass r-card relative overflow-hidden p-5">
      ...
      <div className="space-y-3 relative">
        {milestones.slice(0, 2).map((m, i) => {
          ...
          return (
            <div key={i}>
              <p className="t-body text-slate-900 dark:text-white font-medium">
...
function WalletCard({ balance, feesPaid }: { balance: number; feesPaid: number }) {
  ...
  return (
    <div className="glass r-card relative overflow-hidden p-5">
      ...
      <h3 className="t-micro font-black text-amber-600 dark:text-amber-300 uppercase tracking-[3px] mb-3 flex items-center gap-1.5 relative">
