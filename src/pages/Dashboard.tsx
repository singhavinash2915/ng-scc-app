...
function LiveScorecardWidget({ match }: { match: { id: string; ch_match_id?: string | null; opponent?: string | null; venue?: string; date?: string; overs_per_innings?: number | null; players_per_side?: number | null } }) {
...
export function Dashboard() {
  ...
  // Top 5 MOM winners: this season AND career, side by side.
  //
  // Neither number alone is the answer. "1" in September says nothing about who
  // tends to win these; "11" says nothing about this season. Showing the season
  // figure with the career one behind it means the board reads as the career
  // order in September — which is honest — and starts moving the moment
  // somebody wins one.
  //
  // Everyone with a MOM to their name is a candidate, so a player who has won
  // one this season can't be missing from a list built off career counts.
  const topMOMs = useMemo(() => {
    const ids = new Set([...Object.keys(momAllTime), ...Object.keys(momCounts)]);
    return [...ids]
      .map(memberId => ({
        member: members.find(m => m.id === memberId),
        count: momCounts[memberId] ?? 0,
  ...
  const tickerItems = useMemo(() => {
    ...
    if (topMOMs.length > 0) {
      // Say which race. "leads MOM race (11)" read as if 11 had been won since
      // September; before the first MOM of a season there is no season race to
      // lead, so the ticker says whose career record leads instead.
      const lead = topMOMs[0];
      items.push(lead.count > 0
        ? `${lead.member!.name} leads the MOM race this season (${lead.count})`
        : `${lead.member!.name} leads all time on MOM awards (${lead.career})`);
    }
              ...
              <div className="flex items-center justify-between mb-3 relative">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-amber-300/80 t-micro font-bold uppercase tracking-[1.5px]">
                    MOM Race · {seasonLabel(CURRENT_SEASON)} / all time
                  </span>
                </div>
                ...
                {topMOMs.map((entry, idx) => (
                  <div key={entry.member!.id} className="flex items-center gap-2.5 py-1.5 border-t border-white/6 first:border-0">
                    ...
                    <span className="flex items-center gap-1 text-amber-300 text-xs font-black tabular-nums">
                      {entry.count}
                      {/* Career behind it, so the season number has a scale. */}
                      <span className="t-micro font-bold text-amber-300/45">· {entry.career}</span>
                      {idx === 0 && entry.count > 0 && <span className="t-micro">🏆</span>}
                    </span>
                  </div>
                ))}
              </div>
