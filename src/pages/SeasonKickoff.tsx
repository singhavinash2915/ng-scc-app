  ...
  return (
    <div className="min-h-screen aurora-bg">
      ...
      <div className="p-4 lg:p-8 max-w-3xl mx-auto space-y-4">
        ...
        {/* ── HERO + COUNTDOWN ─────────────────────────────────────────────── */}
        <div className="relative overflow-hidden r-card p-6 text-center text-white shadow-xl"
          ...
          <div className="inline-flex items-center gap-1.5 bg-white/15 rounded-full px-3 py-1 t-meta font-black uppercase tracking-[2px]">
        ...
        <div className="glass r-card p-4">
          <p className="t-meta font-black uppercase tracking-[2px] text-amber-600 mb-3 flex items-center gap-1.5">
            <Trophy className="w-4 h-4" /> Defending champions · {`2025-26`}
          </p>
          <div className="flex gap-2.5 overflow-x-auto pb-1">
            {champions.map(c => (
              <Link key={c.key} to="/awards" className="flex-shrink-0 w-28 r-card bg-white/70 border border-amber-100 p-3 text-center hover:shadow-md transition">
                ...
                <p className="t-meta font-bold text-slate-800 leading-tight mt-0.5 truncate">{c.shownName.split(' ')[0]}</p>
                <p className="t-micro text-slate-400 leading-tight">{c.title}</p>
              </Link>
            ))}
          </div>
        </div>
        ...
        {league.upcoming.length > 0 && (
          <div className="glass r-card p-4">
            <p className="t-meta font-black uppercase tracking-[2px] text-primary-600 mb-3 flex items-center gap-1.5">
              <CalendarDays className="w-4 h-4" /> The road ahead
            </p>
            <div className="space-y-1.5">
              {league.upcoming.slice(0, 6).map(m => (
                <div key={m.id} className="flex items-center gap-3 r-card bg-white/60 px-3 py-2">
                  ...
                  <Link to="/league" className="t-micro font-bold text-primary-500">League →</Link>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ── REVENGE METER ────────────────────────────────────────────────── */}
        {revenge.length > 0 && (
          <div className="r-card p-4 text-white" style={{ background: 'linear-gradient(120deg,#7f1d1d,#b91c1c 60%,#ea580c)' }}>
            <p className="t-meta font-black uppercase tracking-[2px] text-white/90 mb-1 flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {revenge.map(([opp, r]) => (
                <div key={opp} className="r-card bg-white/10 px-3 py-2.5 flex items-center justify-between">
        ...
        <div className="glass r-card p-5">
          <p className="t-meta font-black uppercase tracking-[2px] text-violet-600 mb-1 flex items-center gap-1.5">
            ...
            <div className="space-y-2.5">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {([['🏏 Top run-scorer', topScorer, setTopScorer], ['🎯 Top wicket-taker', topWkt, setTopWkt], ['👑 Season MVP', mvp, setMvp]] as const).map(([label, val, set]) => (
                  <div key={label}>
                    <p className="t-micro font-bold uppercase tracking-wide text-slate-400 mb-1">{label}</p>
              ...
              <div className="flex items-end gap-2">
                <div className="flex-1">
                  <p className="t-micro font-bold uppercase tracking-wide text-slate-400 mb-1">🏆 League wins ({league.totalFixtures} fixtures)</p>
                ...
                <button onClick={saveSlip} disabled={slipSaving || predictions.tableMissing}
                  className="r-control bg-violet-600 hover:bg-violet-700 disabled:opacity-40 text-white font-bold text-sm px-5 py-2.5">
        ...
        {/* ── MY SEASON GOALS ──────────────────────────────────────────────── */}
        <div className="glass r-card p-5">
          <p className="t-meta font-black uppercase tracking-[2px] text-emerald-600 mb-1 flex items-center gap-1.5">
          ...
          {!myId ? (
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <p className="t-micro font-bold uppercase tracking-wide text-slate-400 mb-1">🏏 Runs target</p>
                  <input type="number" min={0} value={goalRuns} onChange={e => setGoalRuns(e.target.value)} placeholder="e.g. 600" className={selectCls} />
                </div>
                <div>
                  <p className="t-micro font-bold uppercase tracking-wide text-slate-400 mb-1">🎯 Wickets target</p>
              ...
              <button onClick={saveGoals} disabled={goalSaving || goalsApi.tableMissing}
                className="w-full r-control bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 text-white font-bold text-sm py-2.5">
              ...
              {myGoal && (
                <div className="space-y-2 pt-1">
                  ...
                  {!seasonStarted && <p className="t-meta text-slate-400">Season hasn't started — your chase begins at the first ball. 🏁</p>}
                </div>
              )}
            </div>
          )}
            ...
            <div className="mt-4 pt-3 border-t border-slate-100">
              ...
              <div className="flex flex-wrap gap-1.5">
                {goalsApi.goals.map(g => (
                  <span key={g.id} className="t-meta bg-emerald-50 text-emerald-700 rounded-full px-2.5 py-1 font-semibold">
        ...
        {values.length > 0 && (
          <div className="glass r-card p-5">
            <p className="t-meta font-black uppercase tracking-[2px] text-amber-600 mb-1 flex items-center gap-1.5">
            ...
            <div className="space-y-1.5">
              {values.slice(0, 12).map((v, i) => (
                <div key={v.member.id} className="flex items-center gap-2.5 r-card bg-white/60 px-3 py-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="t-micro text-slate-400">{v.discipline} · rating {v.rating}</p>
                  </div>
                  <span className={`t-micro font-black uppercase rounded-full px-2 py-0.5 ${v.tier.cls}`}>{v.tier.emoji} {v.tier.label}</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Link to="/fantasy" className="r-card p-4 text-white shadow-lg block"
          ...
          <div className="r-card p-4 text-white shadow-lg" style={{ background: 'linear-gradient(120deg,#92400e,#d97706)' }}>
...
function GoalBar({ label, cur, target }: { label: string; cur: number; target: number }) {
  ...
  return (
    <div>
      <div className="flex justify-between t-meta font-semibold text-slate-500 mb-0.5">
