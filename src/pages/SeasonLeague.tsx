...
function ResultBadge({ result }: { result: Match['result'] }) {
  ...
  return <span className={`t-micro font-bold uppercase px-2 py-0.5 rounded-full ${map[result] ?? map.draw}`}>{result === 'draw' ? 'No result' : result}</span>;
}
  ...
  return (
    <div className="aurora-bg min-h-screen">
      ...
      <div className="p-4 lg:p-8 space-y-4 max-w-3xl mx-auto">
        ...
        {/* ── Hero / record ─────────────────────────────────────────────── */}
        <div className="glass r-card p-5 relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 text-amber-600 dark:text-amber-200">
            ...
            <span className="t-meta font-black uppercase tracking-[2px]">Club League</span>
          </div>
        ...
        {lg.played.length > 0 && (
          <div className="glass r-card p-5">
            <p className="t-meta font-black uppercase tracking-[2px] text-slate-500 dark:text-white/60 mb-3 flex items-center gap-1.5">
              <TrendingUp className="w-4 h-4" /> Results
            </p>
            <div className="space-y-2">
              {[...lg.played].reverse().map(m => (
                <div key={m.id} className="flex items-center gap-3 r-card bg-white/60 dark:bg-white/5 p-3">
                  <div className="text-center min-w-[54px]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {m.man_of_match && (
                      <p className="t-meta text-amber-600 dark:text-amber-300 flex items-center gap-1 mt-0.5"><Star className="w-3 h-3" fill="currentColor" /> {m.man_of_match.name}</p>
                    )}
                  </div>
        ...
        {(lg.topBatter || lg.topBowler || lg.momLeaders.length > 0) && (
          <div className="glass r-card p-5">
            <p className="t-meta font-black uppercase tracking-[2px] text-amber-600 dark:text-amber-200 mb-3 flex items-center gap-1.5">
              <Crown className="w-4 h-4" fill="currentColor" /> League Leaders
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {lg.topBatter && (
                <div className="flex items-center gap-3 r-card bg-white/60 dark:bg-white/5 p-3">
                  <Avatar member={lg.topBatter.member} />
                  <div className="min-w-0">
                    <p className="t-micro uppercase font-bold text-slate-400 dark:text-white/50">🏏 Most Runs</p>
              ...
              {lg.topBowler && (
                <div className="flex items-center gap-3 r-card bg-white/60 dark:bg-white/5 p-3">
                  ...
                  <div className="min-w-0">
                    <p className="t-micro uppercase font-bold text-slate-400 dark:text-white/50">🎯 Most Wickets</p>
            ...
            {lg.momLeaders.length > 0 && (
              <div className="mt-3">
                <p className="t-micro uppercase font-bold text-slate-400 dark:text-white/50 mb-2">⭐ Man of the Match</p>
