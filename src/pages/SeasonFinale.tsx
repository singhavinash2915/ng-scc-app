...
const Avatar = ({ m, size = 'w-10 h-10', ring = 'border-white/15' }: { m: Member; size?: string; ring?: string }) =>
  m.avatar_url ? (
...
const GRADE_COLOR = (g: string) =>
  ...
  : g.startsWith('B') ? 'text-sky-300 bg-sky-500/12 border-sky-400/25'
  ...
  return (
    <div className="glass rounded-2xl p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-[11px] font-black uppercase tracking-[2px] text-amber-200">Team of the Season</span>
      </div>
      {order.map(role => {
        ...
        return (
          <div key={role} className="mb-3 last:mb-0">
            <p className="text-[9px] font-bold uppercase tracking-[1.5px] text-white/40 mb-1.5">{ROLE_LABEL[role]}</p>
            <div className="space-y-1.5">
              {group.map(p => (
                <Link to={`/profile/${p.member.id}`} key={p.member.id}
                  className="flex items-center gap-3 rounded-xl bg-white/[0.04] border border-white/10 px-3 py-2 hover:bg-white/[0.08] transition-colors">
                  <Avatar m={p.member} size="w-9 h-9" ring={p.isCaptain ? 'border-amber-400/60' : 'border-white/15'} />
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-black text-white truncate flex items-center gap-1.5">
                      ...
                      {p.isVice && <span className="text-[8px] font-black bg-white/20 text-white px-1.5 py-0.5 rounded">VC</span>}
                    </p>
                    <p className="text-[11px] text-gray-400 font-semibold tabular-nums">{p.line}</p>
                  </div>
                </Link>
              ))}
            </div>
...
function AwardCard({ award, revealed, onReveal }: { award: Award; revealed: boolean; onReveal: () => void }) {
  ...
  return (
    <button onClick={onReveal} disabled={revealed}
      ...
      <p className="text-[9px] font-bold uppercase tracking-[1.5px] text-white/50">{award.emoji} {award.title}</p>
      {revealed ? (
        <div className="flex items-center gap-3 mt-2" style={{ animation: 'awardPop 0.5s cubic-bezier(0.34,1.56,0.64,1)' }}>
          ...
          <div className="min-w-0">
            <p className="text-sm font-black text-white truncate">{award.member.name}</p>
            <p className="text-[11px] font-bold text-amber-300 tabular-nums">{award.value}</p>
            <p className="text-[10px] text-gray-400 truncate">{award.blurb}</p>
          </div>
        </div>
      ) : (
  ...
  return (
    <div className="glass rounded-xl overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center gap-3 px-3 py-2.5">
        ...
        <span className="flex-1 text-left text-sm font-black text-white truncate">{rc.member.name}</span>
        ...
        <div className="px-3 pb-3 pt-1 border-t border-white/8">
          <div className="grid grid-cols-4 gap-2 my-2">
            {([['Bat', rc.batting], ['Bowl', rc.bowling], ['Field', rc.fielding], ['Impact', rc.impact]] as const).map(([l, g]) => (
              <div key={l} className="text-center rounded-lg bg-white/[0.04] py-2">
                <p className={`text-sm font-black ${GRADE_COLOR(g).split(' ')[0]}`}>{g}</p>
                <p className="text-[8px] font-bold uppercase tracking-wider text-white/40 mt-0.5">{l}</p>
              </div>
            ))}
          </div>
          <p className="text-[11px] text-gray-300 italic">“{rc.note}”</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="dark aurora-bg min-h-screen">
      ...
      <div className="p-4 lg:p-8 space-y-4 max-w-2xl mx-auto">
        ...
        <div className="glass rounded-2xl p-5 text-center relative overflow-hidden">
          ...
          <Trophy className="w-10 h-10 text-amber-300 mx-auto drop-shadow-[0_0_18px_rgba(251,191,36,0.5)]" fill="currentColor" />
          <h2 className="font-display text-2xl lg:text-3xl font-extrabold text-white mt-2">Season {SEASON} Finale 🏏</h2>
          <p className="text-white/60 text-sm mt-1">{clubWrapped.played} matches · {clubWrapped.winPct}% win rate · MVP {clubWrapped.mvp?.name ?? '—'}</p>
        </div>
        ...
        <div className="glass rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-black uppercase tracking-[2px] text-amber-200">Awards Night</span>
            </div>
            {revealed.size < awards.length && (
              <button onClick={revealAll} className="text-[11px] font-bold text-violet-300 hover:text-violet-200">Reveal all →</button>
            )}
          </div>
        ...
        <div className="glass rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-[11px] font-black uppercase tracking-[2px] text-sky-200">Player Report Cards</span>
          </div>
