...
export function useSeasonFinale(season = '2025-26', prevSeason = '2024-25'): SeasonFinale {
  ...
  return useMemo(() => {
    ...
    const bestField = maxBy(dismissals);
    ...
    const awards: Award[] = [
      ...
      { key: 'field', title: 'Best Fielder', emoji: '🧤', member: bestField ? mOf(bestField.member_id) : null, value: bestField ? `${dismissals(bestField)} dismissals` : '—', blurb: bestField ? `${bestField.fielding_catches} ct · ${bestField.fielding_run_outs} ro` : '' },
