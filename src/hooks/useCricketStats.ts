...
export function useCricketStats(season: string = '2025-26') {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      if (season === 'all') {
        // Overall: for each member pick the row with the most innings (= most complete/career data).
        // We do NOT sum rows because season rows may already contain cumulative stats,
        // which would cause double-counting if summed.
        ...
        for (const row of (data || []) as MemberCricketStats[]) {
          // Skip non-year rows ('all-time', etc.)
          if (!YEAR_SEASON.test(row.season)) continue;

          const existing = byMember[row.member_id];
          // Keep whichever row has the most innings — that is the most complete dataset
