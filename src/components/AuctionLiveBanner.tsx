...
export function AuctionLiveBanner() {
  ...
  return (
    <div className="relative overflow-hidden r-card shadow-2xl text-white alb-root"
      style={{
        background: done
      ...
      <div className="px-5 py-4">
        ...
        <div className="flex items-center justify-between">
          <span className="inline-flex items-center gap-2 t-micro font-black uppercase tracking-[2px]">
            <Radio className={`w-3.5 h-3.5 text-rose-400 ${done ? '' : 'alb-live'}`} />
            {done ? 'Auction complete' : 'SCC League auction · live'}
          </span>
          <span className="t-micro font-bold text-white/50">
        ...
        {!done && current && (
          <div key={current.id} className="alb-pop flex items-center gap-4 mt-4">
            {current.avatar_url ? (
              <img src={current.avatar_url} alt="" className="w-16 h-16 r-card object-cover flex-shrink-0"
                style={{ border: '2px solid rgba(255,255,255,.35)' }} />
            ) : (
              <div className="w-16 h-16 r-card bg-white/15 flex items-center justify-center
                              text-2xl font-black flex-shrink-0">
                {current.name.charAt(0)}
              </div>
            )}
            <div className="min-w-0 flex-1">
              <p className="t-micro font-black uppercase tracking-[2px] text-amber-300/80">On the block</p>
              <p className="font-display text-xl font-extrabold leading-tight truncate">{current.name}</p>
              <p className="t-micro text-white/50">
            ...
            <div className="text-right flex-shrink-0">
              ...
              <p className="t-micro font-bold text-white/60 mt-0.5 h-4">
        ...
        <div className="grid grid-cols-2 gap-2.5 mt-4">
          {(['team1', 'team2'] as TeamKey[]).map(t => (
            <div key={t} className="r-card bg-white/10 border border-white/15 px-3 py-2.5">
              <p className="t-meta font-black truncate" style={{ color: TEAM_COLOR[t] }}>
                {TEAM_EMOJI[t]} {name(t)}
              </p>
              ...
              <div className="flex items-baseline justify-between mt-1.5">
                ...
                <span className="t-micro text-white/50">
        ...
        <Link to="/auction"
          className="mt-3 flex items-center justify-center gap-1.5 r-card bg-white/15
                     border border-white/20 py-2 t-meta font-black">
        ...
        {A.sold.length > 0 && (
          <div className="mt-3 pt-3 border-t border-white/10 space-y-1">
            {[...A.sold].slice(-3).reverse().map(p => (
              <div key={p.id} className="flex items-center gap-2 t-meta">
