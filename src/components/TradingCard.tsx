...
export function TradingCard({ member, stats, momCount, rating, compact = false }: Props) {
  ...
  if (compact) {
    return (
      <div className={`relative overflow-hidden r-card bg-gradient-to-br ${role.gradient} p-3 shadow-lg min-w-[140px]`}>
        <div className="flex items-center gap-2">
          {member.avatar_url ? (
            <img src={member.avatar_url} alt="" className="w-10 h-10 r-card object-cover border border-white/20" />
          ) : (
            <div className="w-10 h-10 r-card bg-white/20 flex items-center justify-center">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className={`t-micro font-bold uppercase tracking-wider ${role.accent}`}>{role.label}</p>
          </div>
          <div className={`w-8 h-8 r-card bg-gradient-to-br ${tier.color} flex items-center justify-center`}>
  ...
  return (
    <div className={`relative overflow-hidden r-card bg-gradient-to-br ${role.gradient} shadow-2xl w-full max-w-[220px]`}>
      {/* Decorative pattern */}
      ...
      <div className="absolute top-2 right-2 z-10">
        <div className={`px-2 py-1 r-card bg-gradient-to-r ${tier.color} shadow-lg`}>
          <p className="t-micro font-black tracking-wider">{tier.label}</p>
      ...
      <div className="px-4 pt-2 pb-1">
        ...
        <div className="flex items-center gap-2">
          <span className={`t-micro font-bold uppercase tracking-[2px] ${role.accent}`}>{role.label}</span>
          {member.jersey_number && (
            <span className="text-white/40 t-micro font-bold">#{member.jersey_number}</span>
          )}
        </div>
      </div>
      ...
      {/* Stats grid */}
      {s && (
        <div className="grid grid-cols-3 gap-px mx-3 mb-3 mt-1 bg-white/10 r-card overflow-hidden">
...
function StatCell({ label, value, highlight }: { label: string; value: string | number; highlight?: boolean }) {
  return (
    <div className="bg-black/30 px-1.5 py-1.5 text-center">
      <p className={`t-micro font-black tabular-nums ${highlight ? 'text-amber-300' : 'text-white'}`}>{value}</p>
